...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      const { id } = route.params // vue3取参数
      const { data } = await getDetail(id)
      // console.log(data);
      state.detail = data
      store.dispatch('updateCart') // 让页面出现就发起请求，保证角标一直存在
    })
    ...
    const count = computed(() => {
      return store.state.cartCount // vue3的写法
    })
